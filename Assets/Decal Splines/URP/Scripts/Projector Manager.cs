...
namespace DecalSplines
{
    ...
    //Class used to manage the decal projectors attached to the segment.
    public class ProjectorManager : MonoBehaviour
    {
...
#if UNITY_EDITOR 
            Undo.RegisterCreatedObjectUndo(projectorsObject, "Created projector manager");
...
#if UNITY_EDITOR
            Undo.RegisterCompleteObjectUndo(manager, "Created projector manager");
#endif
            return manager;
        }
        ...
        private DecalProjector SpawnProjector(Vector3 pos, Vector3 nextPos, DecalSplineStyle style, Transform parent)
        {
            if (segment != null)
            {
                ...
                //#if UNITY_EDITOR
                //                Undo.RegisterCompleteObjectUndo(projector, "Created projector");
                //#endif
                return projector;
            }
            return null;
        ...
        {
            List<DecalProjector> excessProjectors = projectorList.GetRange(0, -trimAmount);
            foreach (DecalProjector proj in excessProjectors)
            {
                if (proj != null)
        ...
        public void Destroy(bool undoable = false)
        {
#if UNITY_EDITOR
            if (undoable && !Application.isPlaying)
            {
                Undo.RegisterCompleteObjectUndo(this, "Removed Projector manager");
        ...
        //Set the fadeFactor for individual decal projectors
        private void UpdateTransperancies()
        {
            if (segment != null && !segment.IsLast())
            {
                Vector3[] points = segment.GetCurvePoints();
                float curveLenght = segment.GetCurveLength();

                float distAlongCurve = 0;
                float startTransperancy = segment.Transparency;
                float endTransperancy = 1f;

                ISplineSegment nextSegment = segment.next;
                if (segment.next.GetType() == typeof(DecalSplineSegment))
                {
                    endTransperancy = ((DecalSplineSegment)nextSegment).Transparency;
                }
                ...
                for (int i = 0; i < points.Length - 1; i++)
                {
                    if (curveLenght > 0)
                    {
                        //blend fade along the curve based on the end points fade values and then multiply by the Main Decal Spline FadeFactor.
                        distAlongCurve += Vector3.Distance(points[i], points[i + 1]);
                        float blendPercent = distAlongCurve / curveLenght;
                        projectors[i].fadeFactor = math.lerp(startTransperancy, endTransperancy, blendPercent)* segment.ParentDecalSpline.FadeFactor;
                    }
                    else
                        projectors[i].fadeFactor = endTransperancy* segment.ParentDecalSpline.FadeFactor;
                }
            }
        }
        ...
        private void UpdateProjectorBounds()
        {
            if (segment != null)
            {
                ...
                for (int i = 0; i < points.Length - 1; i++)
                {
                    ...
                    float width = segment.style.Width * segment.ParentDecalSpline.WidthScalar;
                    Vector3 next2D = new Vector3(nextPos.x, 0, nextPos.z);
                    Vector3 pos2D = new Vector3(pos.x, 0, pos.z);
