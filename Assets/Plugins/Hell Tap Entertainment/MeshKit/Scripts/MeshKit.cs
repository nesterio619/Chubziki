...
namespace HellTap.MeshKit {
	...
	public class MeshKit : MonoBehaviour {
		...
		// Constants
		const int maxVertices16 = 65534;		// Unity's upper limit of vertices that can be combined ( max value of 16bit int )
		const uint maxVertices32 = 4294967290;	// Max value for 32bit int (minus 5 for good measure, should be 4294967295 )
		...
		public static MeshKit com {	// <- Here we can call MeshKit.com.FunctionName
			get {
				...
				if( _instance == null ) {
					...
					// FindObjectsOfType was deprecated in Unity 2023.1
					#if UNITY_2023_1_OR_NEWER

						// If there is more than 1 MeshKit Manager in the scene, warn users
						if( FindObjectsByType<MeshKit>(FindObjectsSortMode.None).Length > 1 ){
						...
						_instance = FindFirstObjectByType<MeshKit>();
...
// -> MESH KIT v1.x
		...
		public static Mesh RebuildMesh( Mesh m, bool optionStripNormals, bool optionStripTangents, bool optionStripColors, bool optionStripUV2, bool optionStripUV3, bool optionStripUV4, bool optionStripUV5, bool optionStripUV6, bool optionStripUV7, bool optionStripUV8, bool optionRebuildNormals, bool optionRebuildTangents, float rebuildNormalsAngle = -1 ){
			...
			if( m != null ){
				...
				// BUGFIX: Check for 32-bit meshes (8/10/2024)
				if( m.indexFormat == UnityEngine.Rendering.IndexFormat.UInt32 || 
					m.vertices.Length > 65534 || 
					m.triangles.Length > 65534
				){
	...
	// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		// API SIDE FUNCTION - MeshKit.CombineChildren(args); (this version converts from int to uint)
		public static void CombineChildren( GameObject go, bool optimizeMeshes, int createNewObjectsWithLayer, string createNewObjectsWithTag, bool enabledRenderersOnly, bool createNewObjectsWithMeshColliders, bool deleteSourceObjects, bool deleteObjectsWithDisabledRenderers, bool deleteEmptyObjects, int userMaxVertices = maxVertices16
		){
			MeshKit.com.StartCoroutine( MeshKit.com.CombineChildrenAtRuntime(go, optimizeMeshes, createNewObjectsWithLayer, createNewObjectsWithTag, enabledRenderersOnly, createNewObjectsWithMeshColliders, deleteSourceObjects, deleteObjectsWithDisabledRenderers, deleteEmptyObjects, (uint)userMaxVertices) );
		}

		// API SIDE FUNCTION - MeshKit.CombineChildren(args);
		public static void CombineChildren( GameObject go, bool optimizeMeshes, int createNewObjectsWithLayer, string createNewObjectsWithTag, bool enabledRenderersOnly, bool createNewObjectsWithMeshColliders, bool deleteSourceObjects, bool deleteObjectsWithDisabledRenderers, bool deleteEmptyObjects, uint userMaxVertices = maxVertices16
		){
				...
				foreach(Material m in combines.Keys){
					...
					if( totalVertsCount < userMaxVertices ){
						...
						*/
						...
						// START OF V3 [RUNTIME VERSION IS DIFFERENT]

							// Create a combined mesh using a new variable (avoids errors!)
							Mesh newMesh = new Mesh();
							newMesh.Clear();
							newMesh.name = "Combined " + m.name + "  ["+m.shader.name+"]";	// <- Same as GameObject
							...
							// Combine the meshes into the newly created mesh
							newMesh.CombineMeshes(combines[m].ToArray(), true, true );

							// Now create the meshfilter and setup the mesh
							MeshFilter filter = newGO.AddComponent<MeshFilter>();
						...
						foreach( CombineInstance countCI in combines[m].ToArray() ){
							...
							//	filter.mesh.CombineMeshes( newCombineArr, true, true );
								...
								// START OF V3 [RUNTIME VERSION IS DIFFERENT]
									...
									Mesh newMesh = new Mesh();
									newMesh.Clear();
									newMesh.name = "Combined_" + m.name + "_"+arraysCreated.ToString() +"  ["+m.shader.name+"]";	// <- Same as GameObject
		...
		// We fixed the spelling of this function in v3.0 so lets forward it to the new one for compatibility.
		// Upgraded typo version => Forwards to new Function
	    public static void SeperateMeshes( GameObject go, bool onlyApplyToEnabledRenderers, bool stripUnusedVertices ){
	    	SeparateMeshes( go, onlyApplyToEnabledRenderers, stripUnusedVertices );
	    }
...
// -> MESH KIT v2.0 / v4.0
		...
		public static Mesh DecimateMesh( SkinnedMeshRenderer smr, float quality, bool recalculateNormals, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false, bool preserveSurfaceCurvature = false, bool enableSmartLink = true ){
			if( smr != null && smr.sharedMesh != null ){
				...
				// -> START OF DECIMATION V2

					// Create our mesh simplifier and setup our entire mesh in it
					var meshSimplifier = new HellTap.UnityMeshSimplifier.MeshSimplifier();
					meshSimplifier.Initialize(mesh);

					// Cache the simplification options and mofify them
					var simplificationOptions = meshSimplifier.SimplificationOptions;
					...
					simplificationOptions.EnableSmartLink = enableSmartLink;
					...
					Mesh decimatedMesh = meshSimplifier.ToMesh();


				// <- END OF DECIMATION V2
		...
		public static Mesh DecimateMesh( MeshFilter mf, float quality, bool recalculateNormals, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false, bool preserveSurfaceCurvature = false, bool enableSmartLink = true ){
		...
		public class AutoLODSettings {
			...
			// NOTES: quality and distance is in a different order to LODSettings in LODGenerator
			public AutoLODSettings( float lodDistancePercentage, float quality, SkinQuality skinQuality, bool receiveShadows, ShadowCastingMode shadowCasting, MotionVectorGenerationMode motionVectors, bool skinnedMotionVectors, LightProbeUsage lightProbeUsage = LightProbeUsage.BlendProbes, ReflectionProbeUsage reflectionProbeUsage = ReflectionProbeUsage.BlendProbes, string tag = _UNTAGGED, int layer = 0)
			{
		...
		// OBSOLETE METHODS -> Show obsolete messages because the culling Distance is no longer required
		[System.Obsolete("Remove the cullingDistance float as it is not longer required for the AutoLOD method. The final LOD level sets culling automatically.")]
		public static void AutoLOD( GameObject go, AutoLODSettings[] levels, float cullingDistance = 1f, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false, bool preserveSurfaceCurvature = false, bool enableSmartLink = true ){

			// Recreate the needed arguments and send it to the main method below
			AutoLOD( go, AutoLODSetttingsToLODSettings( levels ), preserveBorders, preserveSeams, preserveFoldovers, preserveSurfaceCurvature, enableSmartLink );
		}

		[System.Obsolete("Remove the cullingDistance float as it is not longer required for the AutoLOD method. The final LOD level sets culling automatically.")]
		public static void AutoLOD( GameObject go, LODSettings[] levels, float cullingDistance = 1f, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false, bool preserveSurfaceCurvature = false, bool enableSmartLink = true ){

			// Recreate the needed arguments and send it to the main method below
			AutoLOD( go, levels, preserveBorders, preserveSeams, preserveFoldovers, preserveSurfaceCurvature, enableSmartLink );
		}
		...
		public static void AutoLOD( GameObject go, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false, bool preserveSurfaceCurvature = false, bool enableSmartLink = true ){
			...
			AutoLOD( go,
				...
				preserveBorders, preserveSeams, preserveFoldovers, preserveSurfaceCurvature, enableSmartLink );
		}
		...
		public static void AutoLOD( GameObject go, AutoLODSettings[] levels, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false, bool preserveSurfaceCurvature = false, bool enableSmartLink = true ){
		...
		public static void AutoLOD( GameObject go, LODSettings[] levels, bool preserveBorders = false, bool preserveSeams = false, bool preserveFoldovers = false, bool preserveSurfaceCurvature = false, bool enableSmartLink = true ){
			...
			// ----------------------------------------------
			//	NOTE: OLD MESHKIT LOD SYSTEM STARTS HERE
			// ----------------------------------------------
			...
			*/
			...
			// -----------------------------------------------
			//	NOTE: NEW SYSTEM FOR MESHKIT v4.0 STARTS HERE
			// -----------------------------------------------

			// ===============================================
			//	SETUP SIMPLIFICATION OPTIONS
			// ===============================================
			...
			LODGroup createdLODGroup = HellTap.UnityMeshSimplifier.MeshKitLODGeneratorV2.GenerateLODs( 
				go, ConvertLODSettingsToLODLevels(levels), true, simplificationOptions
			);
				...
				if( lods.Length == levels.Length ){
					for( int i = 0; i < lods.Length; i++ ){
						foreach( Renderer r in lods[i].renderers ){
							if( r != null ){
								r.gameObject.layer = levels[i].layer;
								r.gameObject.tag = levels[i].tag;
							}
						}
					}
				} else {
		...
		////////////////////////////////////////////////////////////////////////////////////////////////
		//	[HELPER] CONVERT LODSETTINGS TO LODLEVELS
		//	Converts the structs so they can be used in the new Decimation System
		////////////////////////////////////////////////////////////////////////////////////////////////

		public static UnityMeshSimplifier.LODLevel[] ConvertLODSettingsToLODLevels( LODSettings[] lodSettings ){
...
// -> MESH KIT v3.0
		...
		////////////////////////////////////////////////////////////////////////////////////////////////
		//	COMBINE SKINNED MESH RENDERERS (Multiple Materials)
		//	Runtime version of combining a GameObject's children into a single SkinnedMeshRenderer
		//	This runtime version combines the materials but does not create an atlas.
		////////////////////////////////////////////////////////////////////////////////////////////////

		// Combines Skinned Mesh Renderers
		public static void CombineSkinnedMeshRenderers( 
			GameObject go, 													// GameObject to use
			bool onlyCombineEnabledRenderers, 								// Only combine renderers that are enabled
			bool destroyObjectsWithDisabledRenderers						// Destroy any objects that have disabled renderers
		){
			// Make sure a GameObject was supplied
			if( go != null ){

				// Run the method
				if( MeshKitSkinnedMeshRendererUtility.CombineSkinnedMeshRenderersNoAtlas( go, !onlyCombineEnabledRenderers, destroyObjectsWithDisabledRenderers ) == false ){
					Debug.LogWarning( "MESHKIT: There was a problem combining Skinned Mesh Renderers on GameObject: " + go.name );
				}

			// Show Warning log
			} else {
				Debug.LogWarning( "MESHKIT: No GameObject supplied to combine Skinned Mesh Renderers.");
			}
		}

		////////////////////////////////////////////////////////////////////////////////////////////////
		//	SEPARATE SKINNED MESH RENDERERS (Multiple Materials)
		//	Runtime version of seperating a GameObject with a SkinnedMeshRenderer into multiple objects
		////////////////////////////////////////////////////////////////////////////////////////////////

		public static void SeparateSkinnedMeshRenderer( 
			GameObject go, 													// GameObject to use
			bool destroyOriginalSkinnedMeshRenderer = false, 				// When done, destroy the original SkinnedMeshRenderer
			bool inactivateChildSkinnedMeshRendererGameObjects = true, 		// When done, inactivate the GameObjects of any original child SkinnedMeshRenderers
			bool inactivateChildMeshRendererGameObjects = false, 			// When done, inactivate the GameObjects of any original child MeshRenderers
			bool destroyAllChildrenWithDisabledRenderers = false 			// Instead of inactivating the objects above, destroy them instead.
		){

			// Make sure a GameObject was supplied
			if( go != null ){

				// Run the method and show a warning message if something goes wrong
				if( MeshKitSkinnedMeshRendererUtility.SeparateSkinnedMeshRenderer( go, destroyOriginalSkinnedMeshRenderer, inactivateChildSkinnedMeshRendererGameObjects, inactivateChildMeshRendererGameObjects, destroyAllChildrenWithDisabledRenderers ) == false
				){
					Debug.LogWarning( "MESHKIT: There was a problem separating a Skinned Mesh Renderer on GameObject: " + go.name );
				}

			// Show Warning log
			} else {
				Debug.LogWarning( "MESHKIT: No GameObject supplied to separate Skinned Mesh Renderer.");
			}
		}
