...
namespace MelenitasDev.SoundsGood.Editor
{
    public class AudioCreatorEditorWindow : EditorWindow
    {
        ...
        private void CompressionPresetLabel ()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Random Pitch:", titleLabelStyle, GUILayout.Width(125), GUILayout.Height(15));
            isPitchShiftEnabled = EditorGUILayout.Toggle(isPitchShiftEnabled);
            EditorGUILayout.EndHorizontal();
            GUILayout.Box("Random pitch shift function for more lively sound of each effect. " +
                          "If the checkbox is enabled, the sound will be played with a random pitch in the range from 95% to 105%.", greyBoxLabelStyle);
        }
            ...
            if (GUILayout.Button(buttonTitle, GUILayout.Height(30)))
            {
                ...
                if (currentSection == Sections.Sounds)
                    creationSuccess = soundDataCollection.CreateSound(validAudioClips.ToArray(), tag, 
                        compressionPreset, forceToMono, out resultMessage, isPitchShiftEnabled);
                else
