...
#ifdef USE_COMPLEX_T
					half4 splat_control1 = tex2Dlod(_Control1, float4(patch[0].uv, 0, 0));
#endif
#ifdef USE_COMPLEX_T
					if (
						splat_control.r * (1 - _Metallic0) + splat_control.g * (1 - _Metallic1) + splat_control.b * (1 - _Metallic2) + splat_control.a * (1 - _Metallic3)
						+ splat_control1.r * (1 - _Metallic4) + splat_control1.g * (1 - _Metallic5) + splat_control1.b * (1 - _Metallic6) + splat_control1.a * (1 - _Metallic7)
						< 0.5)
#else
					if (
						splat_control.r * (1 - _Metallic0) + splat_control.g * (1 - _Metallic1) + splat_control.b * (1 - _Metallic2) + splat_control.a * (1 - _Metallic3)
...
_LittleSpecForce = max(0.01,pow(_LittleSpecForce, gamma) * 1.5);
					...
					_NormalScaleArray[7] = _NormalScale7;
								...
								float2 offsetParallax = parallax_offset(_ParallaxValue, i.eye, i.uv * _RockScale,
									_ParallaxMapRock, _ParallaxValueMinSamples, _ParallaxValueMaxSamples);
								...
								hole = tex2D(_CustomTerrainHolesTexture, i.uv).r;
								clip(hole == 0.0f ? -1 : 1);
#ifdef USE_COMPLEX_T
								...
								for (int n = 0; n < maxN; n++)
								{
									...
									{
										offsetParallax += parallax_offset(_ParallaxValue, i.eye, i.uv * _RockScale * terrainOffsetST[n],
											_MaskArray[n], _ParallaxValueMinSamples, _ParallaxValueMaxSamples);
									}
								}
								...
								for (int n = 0; n < maxN; n++)
								{
									sandNormalArray[n] = UnpackNormalScale(_NormalArray[n].Sample(my_linear_repeat_sampler, i.uv * terrainOffsetST[n] * lerp(_SandScale, _RockScale, _MetallicArray[n])), _NormalScaleArray[n] * 2);
								}
								...
								// ROCK NORMAL //
								half3 normalTex = UnpackNormalScale(_NormalArray[0].Sample(my_linear_repeat_sampler, i.uv * _RockScale), _NormalScale);
								for (int n = 0; n < maxN; n++)
								{
								...
								float3 normal = UnpackNormalScale(_BumpMap.Sample(sampler_BumpMap, (i.uv) * _SandNormalScale * _SandScale), _NormalMultiplier * 2).rgb * rockValue - i.normal;
								...
								float3 normalDirectionWithNormal = normalize(i.normalDir) + normalize(i.normalDir) * (lerp(abs(_BumpMap.Sample(sampler_BumpMap, i.uv * _SandNormalScale * _SandScale)) * _NormalMultiplier, tex2D(_NormalTex, i.uv * _RockScale) * _NormalScale, saturate(1 - rockValue)));
#endif
...
#if !UNITY_COLORSPACE_GAMMA
								blended.rgb = lerp(blended.rgb + saturate(newRoughnessTex * _RoughnessStrength) * 0.4 * saturate(lightColor.a), blended.rgb, saturate(alphaRock));
#else
								...
								worldNormal.x = dot(normalDirection.x, lerp(normalTex, UnpackNormalScale(_BumpMap.Sample(sampler_BumpMap, i.uv * _SandScale), _NormalMultiplier).rgb * 3 , rockValue));
								worldNormal.y = dot(normalDirection.y, lerp(normalTex, UnpackNormalScale(_BumpMap.Sample(sampler_BumpMap, i.uv * _SandScale), _NormalMultiplier).rgb * 3 , rockValue));
								worldNormal.z = dot(normalDirection.z, lerp(normalTex, UnpackNormalScale(_BumpMap.Sample(sampler_BumpMap, i.uv * _SandScale), _NormalMultiplier).rgb * 3 , rockValue));
					...
					#ifdef IS_T
								float4 specGloss = pow(tex2D(_SpecGlossMapNew, i.uv * 2 * (_TerrainScale.xz / _Splat0_ST.xy) * _SandScale), _SpecForce) * _SpecMult;
...
#else
								rougnessTex = rougnessTex * saturate(UnpackNormalScale(tex2D(_NormalTex, i.uv * _RockScale), _NormalScale) + 0.25);
#endif
...
#ifdef IS_T
								smallSpec = saturate(smallSpec * saturate(normalTex + 0.8));
#else
								smallSpec = saturate(smallSpec * saturate(UnpackNormalScale(tex2D(_NormalTex, i.uv * _RockScale), 20) + 0.8));
#endif
...
#if !UNITY_COLORSPACE_GAMMA
									specularReflection = specularReflection - lerp(0, saturate(0.200), saturate(saturate(lightColor.a * lightIntensity + lightColor.a * 0.35) * saturate(1 - ripples3 * 4) * calculatedNormal.x * reflect(lightDirection, normalDirection)).x * _NormalRTStrength);
#else
...
#endif
									specularReflection = specularReflection + lerp(0, saturate(0.125), saturate(saturate(lightColor.a * lightIntensity + lightColor.a * 0.35) * saturate(1 - ripples3 * 8) * calculatedNormal.x * reflect(-lightDirection, normalDirection)).x * _NormalRTStrength);
