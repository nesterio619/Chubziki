...
public class PrometeoCarController : MonoBehaviour
{
	...
	*/
	...
	void Start()
	{
		...
		InputManager.OnHorizontalAxis += HorizontalInput;
		InputManager.OnVerticalAxis += VerticalInput;
		InputManager.OnStopOrAlternativeUse += Brake;
	...
	private void NewInput()
    {
		...
		if (MoveVertical > 0)
		{
			...
			GoForward(MoveVertical);
		}
		if (MoveVertical < 0)
		{
			CancelInvoke("DecelerateCar");
			deceleratingCar = false;
			GoReverse(MoveVertical);
		}

		if (MoveHorizontal < 0)
		{
			TurnLeft(MoveHorizontal);
		}
		if (MoveHorizontal > 0)
		{
			TurnRight(MoveHorizontal);
		}
		...
		if (MoveVertical == 0 && !_brake && !deceleratingCar)
		{
			InvokeRepeating("DecelerateCar", 0f, 0.1f);
			deceleratingCar = true;
		}
		if ( MoveHorizontal == 0 && steeringAxis != 0f)
		{
			ResetSteeringAngle();
		}
	...
	private void HorizontalInput(float move)
	{
		MoveHorizontal = move;
	}

	private void VerticalInput(float move)
	{
		MoveVertical = move;
	}
	...
	public void TurnLeft(float direction)
	{
		steeringAxis = steeringAxis - (Time.deltaTime * 10f * steeringSpeed);
		if (steeringAxis < direction)
		{
	...
	public void GoForward(float pushPower)
	{
		...
		if (throttleAxis > pushPower)
		{
			throttleAxis = pushPower;
		}
	...
	public void GoReverse(float pushPower)
	{
		//If the forces aplied to the rigidbody in the 'x' asis are greater than
		//3f, it means that the car is losing traction, then the car will start emitting particle systems.
		if (Mathf.Abs(localVelocityX) > 2.5f)
