	...
	public class MeshLOD : UnityEditor.EditorWindow {
		...
		private static void GenerateLODs( MeshKitAutoLOD target ){
			...
			try {
				...
				*/
				...
				// DECIMATION SYSTEM V2
				EditorUtility.DisplayProgressBar("Generating LODs", "Generating LOD Levels...", 0f);
				target.GenerateLODs( true ); // True = show progress bars
				EditorUtility.ClearProgressBar();
		...
		private static void CreateMeshKitAssetsForLODs( MeshKitAutoLOD target ){
			...
			if( target != null && target.gameObject.GetComponent<LODGroup>() != null ){
				...
				//for( int i = 1; i < lods.Length; i++ ){	// <- Skip LOD0 because that is the primary mesh! (OLD SYSTEM)
				for( int i = 0; i < lods.Length; i++ ){		// <- NEW FOR DECIMATION SYSTEM V2: Also do LOD0!

					// NEW V2 SYSTEM: If this LOD Level had a quality of 1, it will use original meshes so we can skip it
					if( target.Levels[i].quality == 1f ){
						if( MeshKitGUI.verbose ){
							Debug.Log("Skipping LOD level " + i + " because quality is set to 1, so probably using original meshes.");
						}
						continue;
					}

					// Make sure the renderer is ok
					if( lods[i].renderers != null ){
						...
						for( int r = 0; r < lods[i].renderers.Length; r++ ){
							if( lods[i].renderers[r] != null ){
								...
								// ==============================================================
								//	FIGURE OUT IF MESH IS A MESHFILTER OR SKINNED MESH RENDERER
								// ==============================================================
