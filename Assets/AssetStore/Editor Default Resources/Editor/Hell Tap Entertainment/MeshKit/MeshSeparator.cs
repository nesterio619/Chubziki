...
namespace HellTap.MeshKit {
	...
	public class MeshSeparator : EditorWindow {
		     	...
		     	if( mesh.subMeshCount > 1 ){
		     		...
		     		for(int i = 0; i < subMeshCount; i++){
						...
						// Mesh Creation encountered a problem, break the loop.
							...
							Debug.LogWarning("MESHKIT SEPERATOR - Encountered a problem! Breaking loop.");
							break;
						}
		     		}
		...
		public static bool CreateMeshAsset( int i, int[] triangles, bool optimize, int id){
			...
			Debug.Log( "Creating Mesh Asset With ID = " + i );
			...
			// BUGFIX: Check for 32-bit meshes (6/4/2024)
			if( mesh.indexFormat == UnityEngine.Rendering.IndexFormat.UInt32 || 
				mesh.vertices.Length > 65534 || 
				triangles.Length > 65534
			){
		...
		// These features work on Unity 2018.2 and up ...
			...
			if(MeshKitGUI.verbose){
				...
				/*
				// NOTE: This section will crash unity if there are too many triangles, so commented out!
				
				string debugTriangles = string.Empty;
				...
				string debugVerts = string.Empty;
				...
				*/
				...
				// Check Results were copied with the same lengths!
