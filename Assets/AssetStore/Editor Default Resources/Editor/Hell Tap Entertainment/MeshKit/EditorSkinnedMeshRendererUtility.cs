...
/// -> [EDITOR ONLY] HELPER CLASSES
		...
		// SHADER PASS DATA CLASS ( not used on skinned mesh renderers )
...
/// -> [EDITOR ONLY] COMBINE SKINNED MESH RENDERERS (WITH ATLASSING)
		...
		public static bool CombineSkinnedMeshRenderers( GameObject selectedGameObject, MeshKitCombineSkinnedMeshSetup.MaxAtlasSize maximumAtlasSize, List<MeshKitCombineSkinnedMeshSetup.CombineSkinnedMeshRendererSetup> propertyList, bool bakeColorIntoMainTex, string assetRelativePathToSaveFolder
			...
			return CombineSkinnedMeshRenderers( selectedGameObject, maximumAtlasSize, propertyNames, textureFallbacks, bakeColorIntoMainTex, assetRelativePathToSaveFolder, bakeFloatsIntoTextures, bakeColorIntoTextures );
		}
		...
		public static bool CombineSkinnedMeshRenderers( 

			GameObject selectedGameObject,
			MeshKitCombineSkinnedMeshSetup.MaxAtlasSize maximumAtlasSize,
			string[] propertyNames,
			MeshKitCombineSkinnedMeshSetup.MissingTextureFallback[] textureFallbacks,
			bool bakeColorIntoMainTex,
			string assetRelativePathToSaveFolder
			...
			List<SkinnedMeshRenderer> onlyActiveRenderersList = new List<SkinnedMeshRenderer>();
			foreach( SkinnedMeshRenderer smr in SMRs ){
				if( smr.enabled == true ){ onlyActiveRenderersList.Add( smr ); }
			}
			// Then, replace the original array
			SMRs = onlyActiveRenderersList.ToArray();
			...
			foreach ( SkinnedMeshRenderer smr in SMRs ){
				...
				// Check if there was an issue found on the current SkinnedMeshRenderer
				bool issueFoundWithSkinnedMeshRenderer = false;
				...
				if( smr.sharedMesh != null && smr.sharedMesh.subMeshCount > 1 ){
					subMeshesFound = true;
					issueFoundWithSkinnedMeshRenderer = true;
					Debug.LogWarning("The SkinnedMeshRenderer named: " + smr.name + "' uses a mesh with submeshes which cannot be combined. Seperate them first!", smr );
				}

				// If we passed the sharedMesh checks, and we haven't already added this mesh to our uniqueMeshes list, do it now
				if( smr != null && smr.sharedMesh != null && smr.sharedMesh.subMeshCount == 1 && 
					uniqueMeshes.Contains( smr.sharedMesh ) == false
				){	
				...
				*/
				...
				if( smr.sharedMaterial.GetTextureScale("_MainTex") != Vector2.one || smr.sharedMaterial.GetTextureOffset("_MainTex") != Vector2.zero ){
					...
					Debug.LogWarning("MeshKit detected a material named '" + smr.sharedMaterial.name + "' on the gameObject named '" + smr.gameObject.name + " that is using a material with custom texture tiling or offset. MeshKit does not currently support material tiling. Ensure your materials have tiling set to (1,1) and offset to (0,0) if you want to combine them effectively.", smr.gameObject );
				...
				// If no issues were found with this SkinnedMeshRenderer, add the shader to our unique list
				if( issueFoundWithSkinnedMeshRenderer == false ){

					// Track Unique Shaders
					if( uniqueShaders.Contains( smr.sharedMaterial.shader ) == false ){
						uniqueShaders.Add( smr.sharedMaterial.shader );
					}

					// Track unique materials
					if( uniqueMaterials.Contains( smr.sharedMaterial ) == false ){
						uniqueMaterials.Add( smr.sharedMaterial );
					}
				}
			}
			...
			foreach( string propertyName in propertyNames ){
				...
				foreach( Material um in uniqueMaterials ){

					// If this Mesh Renderer has a material which contains one of the property names we want to extract (eg "_MainTex")...
					if( um.HasProperty( propertyName) ){

						// This can happen if the main texture being used is baked into an asset
						if( (Texture2D)um.GetTexture( propertyName ) != null ){

							// Cache the 
							Texture2D propertyTex = (Texture2D)um.GetTexture( propertyName );
							...
							if( DoesTextureImporterSetupContainTexture( ref textureImporterSetups, propertyTex ) == false ){
								...
								if ( imp != null && ( 	imp.isReadable == false || 
														...
														textureFallbacks[propertyIndex] == MeshKitCombineSkinnedMeshSetup.MissingTextureFallback.Normal
													)
								){
			...
			for( int i = 0; i < texturePropertySetup.Length; i++ ){
				...
				if( i == 0 ){ 
					...
					// Setup a placeholder bool to catch any errors in packing textures
					bool packTexturesError = false;
					...
					// NEW FIX for Unity 2022 - Custom AtlasPacker! It returns the atlas Texture and Rects array
					(texturePropertySetup[i].atlas, packedRects, packTexturesError) = SquareAtlasPacker.PackTextures( texturePropertySetup[i].textures, 0, maxAtlasSize );

					// If packing finished successfully, do other checks
					if( packTexturesError == false ){
					...
					// UPDATE: As we're using out custom packer utility, we directly set the packTexturesError bool, so check that first.
					if( packTexturesError == true || packedRects.Length == 0 ){
						...
						Debug.LogError( "MESHKIT: Combine SkinnedMeshRenderer operation cancelled because PackedRects array was not created by Unity (this is usually an indicator of the Max Atlas size being too small).");
						return false;
					}
				...
				currentTextureAtlasFileNameToSave = selectedGameObject.name + texturePropertySetup[i].propertyName + ".png";
			...
			for( int i = 0; i < texturePropertySetup.Length; i++ ){
				...
				// Debug which property we are applying ...
				// Debug.Log( "Adding Property: " + texturePropertySetup[i].propertyName );
				...
				// Special Setups - Specular Maps
				if( texturePropertySetup[i].propertyName == "_SpecGlossMap" ){ 
					mat.EnableKeyword("_SPECGLOSSMAP");												// <- Turn On Specular Gloss Map
					
					// If we've added a specular gloss map, assume this is Specular shader:
					mat.shader = Shader.Find("Standard (Specular setup)");

					// BUGFIX: specular shader not applied the first time for some reason ...
					mat.SetTexture( texturePropertySetup[i].propertyName, texturePropertySetup[i].atlas );
				}
...
/// -> [EDITOR ONLY] SEPARATE SKINNED MESH RENDERER

		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	[EDITOR ONLY] SEPARATE SKINNED MESH RENDERER
		//	Allows us to 'Separate' a SkinnedMeshRenderer ( very useful ). 
		//	NOTE 1: if the mesh contains blendshapes, they will no longer work on the 'separated' meshes.
		//
		...
		// Full Method
		public static void SeparateSkinnedMeshRenderer( GameObject selectedGameObject, bool inactivateChildSkinnedMeshRendererGameObjects, bool inactivateChildMeshRendererGameObjects, bool requireConfirmationInEditor = true ){
			...
			if( MeshAssets.HaveSupportFoldersBeenCreated() == false ){
				...
				EditorUtility.DisplayDialog(	
					"MeshKit Skinned Separator", 
					"Cannot run the Separate action because MeshKit support folders for this scene have not been created! Try saving the scene.",
					"Okay"
				);
				return;
			}
			...
			if( inactivateChildSkinnedMeshRendererGameObjects == true ){
				...
				foreach( SkinnedMeshRenderer childSMR in childSMRs ){
					...
					if( childSMR.gameObject != selectedGameObject ){
						...
						// Track Undo
						Undo.RecordObject ( childSMR.gameObject, "Separate Skinned Mesh Renderer");
			...
			if( inactivateChildMeshRendererGameObjects == true ){
				...
				foreach( MeshRenderer childMR in childMRs ){
					...
					if( childMR.gameObject != selectedGameObject ){
						...
						// Track Undo
						Undo.RecordObject ( childMR.gameObject, "Separate Skinned Mesh Renderer");
			...
			for (int i = 0; i < subMeshCount; i++ ){
				...
				seperatedSkinnedMesh.name = "Separated Skinned Mesh (Material " + i.ToString() + ")";
				...
				GameObject holderGameObject = new GameObject( selectedTransform.name + " Separated (Material " + i.ToString() + ")");
				Undo.RegisterCreatedObjectUndo (holderGameObject, "Separate Skinned Mesh Renderer");
				
				// Reparent the GameObject
				//holderGameObject.transform.SetParent( selectedTransform );
				Undo.SetTransformParent ( holderGameObject.transform, selectedTransform, "Separate Skinned Mesh Renderer");
			...
			//	SkinnedMeshRenderer smr = holderGameObject.AddComponent<SkinnedMeshRenderer>();
				...
				MeshAssets.CreateMeshAsset( seperatedSkinnedMesh, MeshAssets.ProcessFileName(MeshAssets.seperatedMeshFolder, selectedTransform.name, "Separated["+i+"]", false) );
				...
				Undo.RecordObject ( smr, "Separate Skinned Mesh Renderer");

			}

			// Disable the original SkinnedMeshRenderer
			Undo.RecordObject ( selectedSkinnedMeshRenderer, "Separate Skinned Mesh Renderer");
