...
namespace MicroWorldNS.Spawners
{
                    ...
                    for (int i = 0; i < SplatMaps.Length; i++)
                    {
                        ...
                        switch (splatMap.Source)
                        {
                            ...
                            case SplatMapSource.CellType:
                                if (cellType == null)
                                    cellType = Map[Builder.PosToHex(worldPosBlended)].Type?.Name ?? "";
                                if (splatMap.CellType == cellType)
                                    found = true;
                                break;
            ...
            IEnumerator BuildSplatmapDataSmoothed()
            {
                ...
                for (int y = 1; y < size; y++)
                {
                    for (int x = 1; x < size; x++)
                    {
                        ...
                        splatmapData[y, x, splatmapLayer[y - 1, x - 1]] += k;
                    }
    ...
    {
        Default, CellType, Slope, Water, Height, Noise
    }
